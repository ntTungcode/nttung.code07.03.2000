...
public class PitchServiceImpl implements PitchService {
    ...
    public String addPitch(PitchRequest request, Long systemId, Long userId) {
        ...
        try {
            ...
            if(request.getUnitPrices().size() > 0){
                Time min = TimeUtils.getMinTimeOfUnitPrice(request.getUnitPrices());
                Time max = TimeUtils.getMaxTimeOfUnitPrice(request.getUnitPrices());
                if(min.after(pitchSystem.getHiredStart()) || min.before(pitchSystem.getHiredStart())){
                    return "Giờ thấp nhất trong bảng danh sách giá theo khung giờ không trùng với giờ mở cửa của hệ thông sân (" + pitchSystem.getHiredStart() + ").";
                }
                if(max.after(pitchSystem.getHiredEnd()) || max.before(pitchSystem.getHiredEnd())){
                    return "Giờ cao nhất trong bảng danh sách giá theo khung giờ không trùng với giờ đóng cửa của hệ thông sân (" + pitchSystem.getHiredEnd() + ").";
                }
            }
    ...
    @Override
    public void updatePitch(PitchRequest request, Long userId, Long id) {
        try {
            PitchType pitchType = typeRepository.findPitchTypeByName(request.getType());
